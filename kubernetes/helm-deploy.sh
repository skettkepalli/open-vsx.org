...
if [[ -z "${environment}" ]]; then
  printf "ERROR: an environment ('test', 'staging', 'aws-production' or 'production') must be given.\n"
  exit 1
fi
...
if [[ "${environment}" == "staging" ]]; then
  ...
  printf "ERROR: Unknown environment. Only 'test', 'staging', 'aws-production' or 'production' are supported.\n"
  exit 1
fi

# Deployment manifest's Values.environment — aws-production uses values-aws.yaml which sets environment=production
